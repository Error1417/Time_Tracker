...
void User::wrtTxt(){
    ...
    outTxt<<"time: "; 
    outTxt<< (time/100) << (time/10%10) << (time%10) <<endl; 
...
void User::wrtBin(){
    ...
    outBin.write(reinterpret_cast<char *>(&time) , sizeof(int)); 
...
void User::reWrtTxt(long begnFile){
    ...
    long cursor = (begnFile <= 0) ? 0 : (begnFile+charCount); // time is a int which is 4 bits
...
//******************************************************************************
//            Checks if current score is greater than time 
//******************************************************************************

bool User::isHiScore(int score){
    
    if((score > 999 || score < 0)){ return false; }
    // time is formatted for max of 3 digits 
    
    else if(score > time ) { return true; }
